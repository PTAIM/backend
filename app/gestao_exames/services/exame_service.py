...
class ExameService:
    ...
    def enviar_resultado_exame(
        ...
        solicitacao = (
            ...
            .filter(SolicitacaoExame.codigoSolicitacao == codigo_solicitacao)
            .first()
        )
    ...
    def listar_solicitacoes_paciente(
        db: Session, paciente_id: int, status: Optional[StatusSolicitacao] = None
    ...
    def listar_solicitacoes_medico(
        db: Session, medico_id: int, status: Optional[StatusSolicitacao] = None
    ) -> List[SolicitacaoExame]:
    ...
    ) -> Optional[SolicitacaoExame]:
        """Busca solicitação por ID"""
        return (
            db.query(SolicitacaoExame)
            .filter(SolicitacaoExame.id == solicitacao_id)
            .first()
        )

    @staticmethod
    def buscar_solicitacao_por_codigo(
        db: Session, codigo: str
    ) -> Optional[SolicitacaoExame]:
        """Busca solicitação por código"""
        return (
            db.query(SolicitacaoExame)
            .filter(SolicitacaoExame.codigoSolicitacao == codigo)
            .first()
        )

    @staticmethod
    def atualizar_status_solicitacao(
        db: Session, solicitacao_id: int, novo_status: StatusSolicitacao
    ) -> SolicitacaoExame:
    ...
    @staticmethod
    def buscar_resultados_solicitacao(
        db: Session, solicitacao_id: int
    ) -> List[ResultadoExame]:
    ...
    @staticmethod
    def buscar_resultado_por_id(db: Session, exame_id: int) -> Optional[ResultadoExame]:
        return db.query(ResultadoExame).get(exame_id)
    ...
    def listar_resultados_medico(db: Session, medico_id: int) -> List[ResultadoExame]:
        ...
        return (
            db.query(ResultadoExame)
            .join(SolicitacaoExame)
            .filter(SolicitacaoExame.medicoSolicitante == medico_id)
            .order_by(ResultadoExame.dataUpload.desc())
            .all()
        )
    ...
    @staticmethod
    def listar_resultados_funcionario(
